...
delay_days_model = joblib.load('best_model_rf.pkl')
classification_model = joblib.load('random_forest_model_classification.pkl')
financial_model = joblib.load('random_forest_top5_cost_model.pkl')

delay_days_features = joblib.load('Delay_Days_Features_list.pkl')
classification_features = joblib.load('top_features_classification.pkl')
financial_features = joblib.load('random_forest_top5_cost_features.pkl')

financial_scaler = joblib.load('financial_scaler.pkl')  # Make sure you saved this scaler!
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["📦 Delay Days", "🚦 Delay Classification", "💰 Financial Impact"])
            ...
            if errors:
                ...
                try:
                    ...
                    if not ok:
                        ...
                        st.success(f"🚚 Estimated Delay: {pred:.2f} days")
        ...
        if st.button("Predict Shipment Delay"):
            if errors:
                ...
                try:
                    ...
                    st.success(f"🚨 Delay Prediction: {pred} (Probability: {proba:.2%})")
                except Exception as e:
        ...
        if st.button("Predict Financial Impact"):
            if errors:
                ...
                try:
                    ...
                    st.success(f"💸 Predicted Financial Impact: ${pred_usd:,.2f} USD")
                except Exception as e:
                    st.error(f"Prediction error: {e}")
